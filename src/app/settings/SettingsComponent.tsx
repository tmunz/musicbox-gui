    ...
    {Object.entries(appState.visualization?.settings ?? {}).map(([sectionKey, section]) => (
      <section key={sectionKey}>
        ...
        {Object.entries(section as Settings).map(([key, setting]) => (
          <NumberSettingsComponent key={key} setting={setting as Setting<number>} onChange={(value: number) => dispatch({ type: VisualizationAction.UPDATE_VISUALIZATION_SETTINGS_VALUE, section: sectionKey, key, value })} />
        ))}
      </section>
    ))}
  </div>;
